...
int main(void){
    pstring_t empty_str;
    int ret = pstring_init_empty(&empty_str);
    if (ret == PSTR_OK){
    ...
    int len_ret = pstring_init_from_length(&str_with_length, 10);
    if (len_ret == PSTR_OK){
    ...
    char buffer[] = "abcde";
    int buf_ret = pstring_init_from_buffer(&str_from_buf, buffer, 6);
    if (buf_ret == PSTR_OK){
    ...
    int cstr_ret = pstring_init_from_cstring(&str_from_c_str, cstr);
    if (cstr_ret == PSTR_OK){
    ...
    int too_long_code = pstring_init_from_cstring(&str_too_long, too_long);

    if (too_long_code == PSTR_ERROR_TOO_LONG){
        printf("It was too long! Nothing created!\n");
    }

    pstring_destroy(&empty_str);
    pstring_destroy(&str_with_length);
    pstring_destroy(&str_from_buf);
    pstring_destroy(&str_from_c_str);
}