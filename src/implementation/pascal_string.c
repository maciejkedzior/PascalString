...
int init_pstring_from_length(pstring_t *s, uint8_t length){
    assert(s != NULL);
    if (length == 0) return init_pstring_empty(s);
...
int init_pstring_from_buffer(pstring_t *s, const char* buffer, uint8_t len){
    ...
    int ret_code = init_pstring_from_length(s, len);    // invariants are checked inside the initializer so no need to check them here
...
int init_pstring_from_cstring(pstring_t *s, const char *c_str){
    ...
    int ret_code = init_pstring_from_length(s, (uint8_t)c_str_len);
